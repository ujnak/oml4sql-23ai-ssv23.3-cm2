...
CREATE OR replace VIEW mdb_rdemo_v AS 
  SELECT cust_id, age, soccer, cricket,
    cust_marital_status, tennis, household_size, occupation,
    yrs_residence, unknown FROM mining_data_build_v;
...
BEGIN
  ...
  -- Configure model settings using SETTING_LIST
...
column exp_upper_coeff_limit format 9999990.999
  SELECT *
  FROM DM$VDGLMR_SH_REGR_SAMPLE
 WHERE attribute_name in ('AFFINITY_CARD','CRICKET','COUNTRY_NAME')
